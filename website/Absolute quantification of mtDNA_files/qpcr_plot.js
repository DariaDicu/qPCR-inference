...
function get_normal(mean, variance) {
   var d = []
   for (var i = 0; i < 101; i++) {
      var x = i*0.01
      var y = Math.exp(-(x-mean)*(x-mean)/(2*variance))
      y = y/Math.sqrt(2*Math.PI*variance)
      d.push([x, y])
   }
   console.log(d)
   return d
...
var heteroplasmy_demo = Highcharts.chart('heteroplasmy_demo', {
    chart: {
        type: 'areaspline'
    },
    title: {
        text: 'Heteroplasmy distributions with same mean, different variance', 
    },
    xAxis: {
        allowDecimals: true,
        gridLineWidth: 1,
    ...
    series: [{
         name: 'Heteroplasmy distribution 1',
         data: get_normal(0.65, 0.02),
         width: 1,
         pointStart: 1,
         tooltip: {
             valueDecimals: 2
         }},{
         name: 'Heteroplasmy distribution 2',
         data: get_normal(0.65, 0.001),
         width: 1,
         pointStart: 1,
         tooltip: {
             valueDecimals: 2
