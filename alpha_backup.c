...
const uint64_t dilution_series[4] = {100000, 10000, 1000, 100};
...
uint64_t binomial_sample(double p, uint64_t x) {
	if ((double)x*p > 9.0*(1-p) && (double)x*(1-p) > 9.0*p) {
		// Approximate with normal and return.
		// Sample from N(0, 1).
		double N_sample = rand_N();
		if (N_sample*sqrt(x*p*(1-p)) + x*p < 0) {
			printf("negative sample!!! overflow!\n");
		}
		//printf("In: %G %" PRIu64 ", out: %"PRIu64, p, x, final_sample);

		// Shift and scale to get N(xp, xp(1-p)).
		return (uint64_t)(N_sample*sqrt(x*p*(1-p)) + x*p);
	}
...
double log_posterior(const double *F, int n, const double *theta) {
	...
	for (int d = 0; d < dilution_series_size; d++) {
		...
		for (int i = 1; i <= n; i++) {
			...
			double max_p, prob;
			for (int k = 0; k < M; k++) {
				x[k] = samples[k] + binomial_sample(p, samples[k]);
			...
			// Assign new weights.
			// Use LSE approximation for total weights.
			double log_total_weights = max_p;
			for (int k = 0; k < M; k++) {
				...
				//if (i == n) printf("Final run w %G %G %G %G\n", w[k], prob, max_p, prob - max_p);
				log_total_weights += exp(w[k]);
			}
			...
			// underflow.
			ll += log_total_weights - log(M) - log(2*M_PI*sigma)/2 + max_p;

			//if (i == n) printf("%"PRIu64" %G\n", x[0], total_weights);
		}
	}
